...
export default function ChatPage() {
  ...
  if (loading) return <div className="p-6 text-foreground">Loading chat...</div>;
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto h-screen flex flex-col relative">
        <div className="flex justify-between p-3 border-b border-border bg-card/80 backdrop-blur-sm">
...
function MessageComposer({ onSend }: { onSend: (msg: string) => void }) {
  ...
  return (
    <div className="border-t border-border p-4 flex gap-2 bg-card/80 backdrop-blur-sm">
      <input
        value={text}
        onChange={(e) => setText(e.target.value)}
        onKeyDown={(e) => e.key === "Enter" && send()}
        className="flex-1 border border-border bg-background text-foreground p-2 rounded"
        placeholder="Type a message..."
      />
      <button onClick={send} className="bg-primary text-primary-foreground px-4 rounded">
        Send
      </button>
    </div>
  );
